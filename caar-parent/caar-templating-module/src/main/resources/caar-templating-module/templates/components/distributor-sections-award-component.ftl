[#assign createNode = model.createJuryNode(cmsfn.asJCRNode(content))!""]
...
[#list sortedList as item]
	...
	[#if contentItem.type?has_content]
		...
		[#if contentItem.type == "jury"]
			[#assign juryOptionIdAnchor = model.getAnchorFromString(contentItem.juryOptionTitle)!""]
		...
		[#if contentItem.type == "lemma"]
			[#assign lemmaOptionIdAnchor = model.getAnchorFromString(contentItem.lemmaOptionIdAnchor)!""]
		...
		[#if contentItem.type == "enrollment"]
			[#assign enrollmentOptionIdAnchor = model.getAnchorFromString(contentItem.enrollmentOptionIdAnchor)!""]
